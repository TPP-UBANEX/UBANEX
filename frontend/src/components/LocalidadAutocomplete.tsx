...
import type { Localidad, ValorGeolocalizacion } from '@/data/types'
import { Loader2, MapPin } from 'lucide-react'
import { cn } from '@/lib/utils'

interface Props {
  value: ValorGeolocalizacion | null
  onChange: (valor: ValorGeolocalizacion | null) => void
  disabled?: boolean
}

/** Input de texto con búsqueda de localidades de Argentina contra la API Georef.
 *  Si no se elige ninguna sugerencia, lo tipeado se guarda como texto libre (sin coordenadas). */
export function LocalidadAutocomplete({ value, onChange, disabled }: Props) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        const resultados = await api.geo.localidades(texto.trim())
  ...
  const elegir = (localidad: Localidad) => {
    onChange({
      id: localidad.id,
      nombre: `${localidad.nombre}, ${localidad.provincia}`,
      provincia: localidad.provincia,
      lat: localidad.lat,
      lon: localidad.lon,
    })
    setTexto(`${localidad.nombre}, ${localidad.provincia}`)
    setAbierto(false)
    setResaltado(-1)
  }
  ...
  const handleChangeTexto = (nuevoTexto: string) => {
    ...
    setAbierto(true)
    setResaltado(-1)
    // Texto libre: se guarda sin coordenadas hasta que se elija una sugerencia.
          ...
          {opciones.length === 0 ? (
            ...
            opciones.map((localidad, idx) => (
              ...
              >
                ...
                <span>{localidad.nombre}, {localidad.provincia}</span>
              </button>
            ))
          )}
        </div>
