...
- **Builders de configuración**: `FormularioBuilderTab`, `TemplateInstitucionalBuilder`, `TemplateCruzadaBuilder`, `TemplateAutoevaluacionBuilder`, `ColumnasTablaEditor`, `ConfigTipoCampoEditor`. `FormularioBuilderTab` y `PlantillaFormularioDetail` tienen un interruptor Editor/Vista previa que monta `VistaPreviaFormulario` (agrupa los campos en tabs por sección con `agruparCamposEnSecciones` y los muestra en solo lectura, más mocks de las tabs fijas Dirección y Presupuesto). `TemplateInstitucionalBuilder` y `TemplateCruzadaBuilder` listan sus categorías con el mismo lenguaje visual que `CamposFormularioEditor`: una única lista full-bleed (`-mx-6 border-y divide-y`) donde la cabecera de cada categoría es la fila destacada (fondo `bg-muted/50`, barra izquierda gris) y sus subcategorías/ítems son filas lisas debajo, sin caja propia. `EvaluacionConfigTab` (por convocatoria) alterna institucional y cruzada con un selector tipo `Tabs` en vez de mostrarlos lado a lado, marca con un punto la pestaña que tiene cambios sin guardar (comparando contra la estructura tal como vino del backend), y junto con los diálogos de `PlantillasEvaluacion` tiene el mismo interruptor Editor/Vista previa por separado para cada formulario, montando `VistaPreviaEvaluacionInstitucional` / `VistaPreviaEvaluacionCruzada` (categorías/subcategorías/checklist o categorías/ítems puntuables en solo lectura, replicando el render real de `Evaluacion.tsx`). Para cargar una plantilla, `EvaluacionConfigTab` usa el mismo botón "Reemplazar por plantilla" en el header y el mismo `SeleccionarPlantillaDialog` que `FormularioBuilderTab` — el diálogo es genérico (`PlantillaSeleccionable`, con `cargarPlantillas`/`detalle`/`onSeleccionar` inyectados por cada llamador) para que ambas pestañas compartan exactamente el mismo componente y posición.
- **Flujo de edición/proyecto**: `NuevoProyectoDialog`, `ResubirProyectoDialog`, `TablaPartidasPresupuesto`, `CampoFormularioInput` / `CampoFormularioLectura`, `TablaCampoFormulario` (campos tipo tabla), `SeleccionarPlantillaDialog`, `ListaCamposFaltantes`, `CampoSugerible`, `SugerirCambioModal`, `SugerenciasTab`, `HistorialTab` (pestaña de historial de trazabilidad del proyecto). `SugerenciasTab` e `HistorialTab` comparten `lib/nombre-campo.ts#nombreCampoSugerencia` para traducir la clave de un campo observado a su etiqueta legible. En `ProyectoDetail`, el modo edición tiene un interruptor **Editar / Vista previa** que renderiza las tabs de presentación en modo lectura con los datos sin guardar, y `lib/exportar-proyecto-pdf.ts` genera el PDF del proyecto (detalle + formulario + presupuesto) con `jsPDF`.
- **Evaluación y adjudicación**: `ProyectoEvaluablePanel`, `EvaluacionesProyectoTab`, `EvaluacionConfigTab`, `EmparejamientoTab`, `AdjudicacionResolucionTab`, `AsignacionEvaluadores`. `lib/exportar-resolucion-pdf.ts` genera el PDF de la resolución de adjudicación emitida, mismo enfoque `jsPDF` que `exportar-proyecto-pdf.ts`.
...
| Alta/baja de directores y evaluadores | `participaciones-convocatoria/` | `AsignacionEvaluadores` |
